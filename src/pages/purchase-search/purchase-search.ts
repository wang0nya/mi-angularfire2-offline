        ...
        this.headerRow = unparsedProducts[0];
         ...
         this.productList = unparsedProducts;
          ...
          purchasesCSV() {
              ...
              this.file.writeFile(this.file.externalRootDirectory, Date.now()+'-purchases-report.csv', blob, { replace: true }).then((entry) => {
              this.toast.show('Purchases Report downloaded', '5000', 'center').subscribe(
                toast => {
                  this.navCtrl.pop();
                }
              );
